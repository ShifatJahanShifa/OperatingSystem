...
long long int resMat[1000][1000];
...
int slave(int fdValue,long long int startRow,long long int endRow)
{
	for(int i=startRow;i<endRow;i++) 
	{
        ...
        size_t bytesWrite=write(fdValue,&resMat[startRow],sizeof(long long)*(endRow-startRow)*column2);
	if(bytesWrite==-1) 
	{
...
int main()
{
	...
	for(int i=0;i<totalChild;i++)
	{
		...
		size_t bytesRead=read(fd[0],&resMat[startRow],sizeof(long long)*numElements);
		if (bytesRead == -1)
		{
